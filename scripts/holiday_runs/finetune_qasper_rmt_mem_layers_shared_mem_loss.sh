...
horovodrun --gloo -np $NP python run_finetuning_scrolls_rmt_mem_loss.py \
        --task_name $TASK_NAME \
        --model_path ../runs/debug/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${INPUT_SEQ_LEN}-${TGT_LEN}-{$MAX_N_SEGMENTS}seg_mem${MEMORY_SIZE}_bs${TBS}_iters${ITERS}_${SEGMENT_ORDERING}_mem_layers_shared_mem_loss_rc${REC_COEF}/run_$N \
