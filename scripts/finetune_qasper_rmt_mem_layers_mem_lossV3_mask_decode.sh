...
MODEL_CLS=modeling_rmt_enc_dec_framework_mem_lossV3_mask_decode:RMTEncoderDecoderForConditionalGeneration
...
MAX_N_SEGMENTSS=(2 2 1 )
MEMORY_SIZES=(10 1 10)

for N in 1 2
do

for REC_COEF in 0.001
do
...
horovodrun --gloo -np $NP python run_finetuning_scrolls_rmt_framework.py \
        --task_name $TASK_NAME \
        --model_path ../runs/debug/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${INPUT_SEQ_LEN}-${TGT_LEN}-{$MAX_N_SEGMENTS}seg_mem${MEMORY_SIZE}_bs${TBS}_iters${ITERS}_${SEGMENT_ORDERING}_mem_layers_mem_lossV3_mask_decode_rc${REC_COEF}/run_$N \
        --memory_forward_implementation ./rmt_utils/memory_layers/ \
