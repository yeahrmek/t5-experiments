...
MODEL_TYPE=decoder
MODEL_CLS=modeling_rmt:RMTDecoderMemoryLayers
...
horovodrun --gloo -np $NP python run_finetuning_lm_rmt.py \
        --task_name $TASK_NAME \
        --model_path ../runs/test/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${INPUT_SEQ_LEN}-${TGT_LEN}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}rw_bs${TBS}_iters${ITERS}_${SEGMENT_ORDERING}_bptt-${K2}_fix_align_mem_layers_shared_freeze_from_cpt/run_$N \
