...
horovodrun --gloo -np $NP python run_finetuning_lm_rmt.py \
        --task_name $TASK_NAME \
        --model_path ../runs/test/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${INPUT_SEQ_LEN}-${TGT_LEN}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}rw_bs${TBS}_iters${ITERS}_${SEGMENT_ORDERING}_bptt-${K2}_mem_layers_w2r_shared_separate_mem_from_cpt/run_$N \
