...
def encoder_memory_forward(
    ...
    for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
        ...
        if self.gradient_checkpointing and self.training:
            ...
            if rmt_parent.memory_layers[i] is None:
                layer_attention_mask = torch.clone(extended_attention_mask)
                layer_attention_mask[:, :, :, rmt_parent.memory_position] = -10000.
...
class RMTEncoderDecoderForConditionalGeneration():
    ...
    def override_encoder_forward(self):
        self.memory_storage = {}
        # identy_layer = lambda x, **kwargs: [x]
        identy_layer = None
        memory_layer = copy.deepcopy(self.model.encoder.block[0])

        self.memory_layers = [identy_layer for _ in range(len(self.model.encoder.block)-1)] + [memory_layer]
