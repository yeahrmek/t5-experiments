...
class RMTEncoderDecoderForConditionalGeneration():
    ...
    def set_params(self, num_mem_tokens, tokenizer, **rmt_config):
        ...
        self.reconstruction_cls = torch.nn.Linear((num_mem_tokens + 1) * self.config.d_model, rmt_config['max_n_segments']).to(device=self.device)
    ...
    def segment_reconstruction_forward(self, segmented, memory_out):
        ...
        # print('checkbox.shape', checkbox.shape)
        appearance = torch.zeros((*checkbox.shape[:2], self.rmt_config['max_n_segments'])).to(device=self.device)
        ...
        checkbox = checkbox.reshape(checkbox.shape[0] * checkbox.shape[1], -1)
        appearance = appearance.reshape(appearance.shape[0] * appearance.shape[1], -1)
        # print('appearance.shape', appearance.shape)

        # print('checkbox.shape', checkbox.shape)
        # print('self.reconstruction_cls', self.reconstruction_cls.weight.shape) 
        cls_out = self.reconstruction_cls(checkbox)

        loss_fct = MSELoss()
        loss = loss_fct(cls_out, appearance)
