...
class RMTDecoderForCausalLM(RMTBaseModel):
    ...
    def extend_word_embeddings(self, num_mem_tokens, tokenizer):
        ...
        self.register_buffer(
            "mem_token_ids", torch.arange(vocab_size, vocab_size + num_mem_tokens)
        )
    ...
    def set_max_n_segments(self, max_n_segments: int) -> None:
        self._actual_max_n_segments = max_n_segments
...
class RMTDecoderMemoryLayers(RMTDecoderForCausalLM):
    ...
    def override_encoder_forward(self, memory_forward_func):
        ...
        new_forward = lambda *args, **kwargs: memory_forward_func(
            *args, **kwargs, rmt_parent=self
        )
        self.model.base_model.forward = types.MethodType(
            new_forward, self.model.base_model
        )

    def add_memory_layers(self):
        memory_layers, share_memory_layers = self.rmt_config.get(
            "memory_layers"
        ...
        if memory_layers is None:
            ...
            if share_memory_layers:
                ...
                self.memory_layers = [
                    copy.deepcopy(self.model.base_model.h[int(l)])
...
class RMTDecoderScaleMem(RMTDecoderMemoryLayers):
    def extend_word_embeddings(self, num_mem_tokens, tokenizer):
        ...
        embeddings.weight.data[-num_mem_tokens:] = (
            embeddings.weight.data[-num_mem_tokens:].normal_(
                mean=0.0, std=embeddings.weight.data.std()
            )
            / 100
            + embeddings.weight.data[tokenizer.eos_token_id]
        )
...
class RMTDecoderMemFromDot(RMTDecoderMemoryLayers):
    def extend_word_embeddings(self, num_mem_tokens, tokenizer):
        ...
        embeddings.weight.data[-num_mem_tokens:] = (
            ...
            / 100
            + embeddings.weight.data[tokenizer.encode(".")[0]]
        )
        self.model.set_input_embeddings(embeddings)
        self.model.tie_weights()
