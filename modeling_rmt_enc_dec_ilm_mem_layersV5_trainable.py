...
class RMTEncoderDecoderForConditionalGeneration():
    ...
    def set_memory(self, memory=None):
        ...
        # fill layer memories 
        for i, lm in enumerate(self.layer_memories):
            self.memory_storage[i] = torch.stack([lm])
    ...
    def extend_word_embeddings(self, num_mem_tokens):
        ...
        self.layer_memories = []
        for ln, layer in enumerate(self.encoder.block):
            mem_weights = torch.rand(num_mem_tokens, self.model.config.hidden_size)
            layer_mem = torch.nn.Parameter(mem_weights, requires_grad=True)
            self.register_parameter(name=f'memory_l{ln}', param=layer_mem)
            self.layer_memories.append(layer_mem)
